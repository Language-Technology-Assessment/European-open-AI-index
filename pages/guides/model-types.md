...
To start, it is useful to look to the models which initiated the recent revolution in the field of AI. Text-based models such as ChatGPT are not constructed in a single step; instead, they undergo at least a two-phase construction. Firstly, a base LLM is developed which stores a model of human language, and subsequently this model is 'fine-tuned' (slightly modified) to serve as an effective chatbot.
